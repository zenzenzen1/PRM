...
import com.example.tourbooking.repository.RoleRepository;
...
import com.example.tourbooking.repository.VehicleRepository;
...
public class SeedDatabase {
    ...
    public void Initialize() {
        ...
        if (userRepository.getAllUser().isEmpty()) {
            ...
            user.setRole_id(Role.EnumRole.MANAGER.getValue());
            user.setLocked(false);
            userRepository.createUser(user);
            User user1 = new User();
            user1.setUserName("user1");
            user1.setPassword("123");
            user1.setRole_id(Role.EnumRole.USER.getValue());
            user1.setLocked(false);
            userRepository.createUser(user1);
        }

        if(statusRepository.getAllStatus().isEmpty()){
            statusRepository.insertAll(
                    Arrays.stream(Status.StatusEnum.values()).map((se) -> new Status(se.getId(), se.getStatusName(), "")).toArray(Status[]::new)
                    );
        }
        ...
        if (tourRepository.getAllTour().isEmpty()) {
            ...
            double[] prices = {99000, 150000, 300000, 750000, 10_000, 120000};
            for (int i = 0; i < 20; i++) {
                Tour tour = new Tour();
